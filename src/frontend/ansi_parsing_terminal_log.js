...
import {
    ...
    getInputHintMarker,
    parseSimpleMessage,
    parseSimpleANSIMessage
} from "./parse_terminal_ansi_message.js";
...
class BeautifiedLogMessage {
    ...
    getParsedMessage() {
        if (!this.parsedMessage) {
            ...
            if (message.textClass.v === TextClass.CONTENT.v && this.parseANSI) {
                console.log("Parsing ANSI message", text)
                const {frag} = parseSimpleANSIMessage(this.container, this, this.beautifiedLogList, this.checkIfMergeWithPrevious(),
                    this.checkIfContinuousMessage());
                if (frag === null) {
    ...
    provideLogInfo({container, beautifiedLogList}) {
        this.beautifiedLogList = beautifiedLogList;
        this.container = container;
        this.previousMessage = null;
        if (beautifiedLogList.length > 0) {
            this.previousMessage = beautifiedLogList[beautifiedLogList.length - 1];
        }
...
const elementIdentifier = (element) => {
    return element.id;
}

export
class SimpleANSIParsingLog {
    ...
    constructor(log) {
        this.log = log;
        log.writeMessageToElement = this.writeMessageToElement;
        this.beautifiedLog = new Map();
    }
    ...
    writeMessageToElement = (element, message) => {
        console.log("Writing message to element", element, message)
        if (!element.classList.contains('logContainer')) {
            throw new Error('HTMLElement must have class "logContainer"');
        }

        const beautifiedLogMessage = new BeautifiedLogMessage(message);

        const containerID = elementIdentifier(element);
        if (!this.beautifiedLog.has(element)) {
            this.beautifiedLog.set(element, []);
        }
        const beautifiedLogList = this.beautifiedLog.get(element)
        beautifiedLogMessage.provideLogInfo({container: element, beautifiedLogList});

        const contentToAppend = beautifiedLogMessage.beautifiedMessage;
        if (contentToAppend === null) {
            console.log("Content to append is null", message);
            return;
        }
        ...
        if (message.password) {
            ...
            messageContentContainer.click();
        }

        if (beautifiedLogMessage.mergeWithPrevious) {
            // hide meta info
            contentToAppend.classList.add('mergedWithPrevious');
        }

        contentToAppend.title = JSON.stringify(message.text);
        beautifiedLogList.push(beautifiedLogMessage);
    }
}